...
class Evaluator:
    ...
    def evaluate(
        ...
        # Load test data
        logger.info(
            "Loading evaluation data", ticker=ticker, start=start_date, end=end_date
        )
        ...
        logger.info("Evaluation results structure", results_keys=list(results.keys()))
    ...
    def _evaluate_agent(self, env: TradingEnv, data: pd.DataFrame) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                "portfolio_values": [self.config["portfolio"]["starting_cash"]],
                "actions": [],
                "metrics": {
                    "total_return": 0.0,
                    "volatility": 0.0,
            ...
            if len(buy_actions) > 0 and len(sell_actions) > 0:
                ...
                for buy_idx in buy_actions:
                    for sell_idx in sell_actions:
                        if sell_idx > buy_idx:
                            trade_return = (
                                portfolio_values[sell_idx] - portfolio_values[buy_idx]
                            ) / portfolio_values[buy_idx]
    ...
    def _create_evaluation_plots(
        self, results: dict[str, Any], data: pd.DataFrame
    ) -> None:
...
def evaluate_agent(
    ...
    results = evaluator.evaluate(
        ticker, start_date, end_date, interval, baseline_strategies
    )
